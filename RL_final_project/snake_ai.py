...
import matplotlib
import platform  # 添加 platform 模块

# 设置matplotlib后端以确保显示
# 修改：针对 macOS 避免使用 TkAgg，改用 MacOSX 后端
try:
    if platform.system() == 'Darwin':
        matplotlib.use('MacOSX')  # macOS 原生后端，更稳定
    else:
        matplotlib.use('TkAgg')   # Windows/Linux 默认尝试 TkAgg
except:
    try:
...
# --- 新增: Apple Silicon MPS 支持 ---
elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
    DEVICE = torch.device("mps")
    print(f">>> 成功启用 Apple Metal 加速 (MPS) - 适用于 Mac M1/M2/M3/M4 <<<")
else:
...
if __name__ == '__main__':
    ...
    print("用法:")
    print("  python snake_ai.py         -> 训练模式")
    print("  python snake_ai.py play    -> 观看AI玩")
    print("  python snake_ai.py human   -> 玩家试玩")
